...
Modified by Sayak Chakraborti ,March 2017 for parallelism using open MP
Compile-gcc guass_p.c -fopenmp -o openmp -O3 -w
Run-./openmp matrix_2000.dat
static=blocked
dynamic=dynamic load balance
static with CHUNK size 1=cyclic 
...
//
void initResult(int nsize)        //here result is the values of X that we want to find
{
  int i;

  X = (double*) malloc(nsize * sizeof(double));   //the X matrix
  assert(X != NULL);
  //initialize X with 0s
  for (i = 0; i < nsize; i++) 
  {
    X[i] = 0.0;
  }
...
void computeGauss(int nsize,int num) 
{
  ...
  omp_set_num_threads(num);   //set the number of threads for the current execution
  
  //Just to print the number of threads and verify
#pragma omp parallel private(tid)                 
  {
     tid=omp_get_thread_num();                 //get the thread ID

    if(tid==0)
    {printf("Number of threads %d\n",omp_get_num_threads());   //the thread with TID 0 prints this 
     }
  }   
    ...
    #pragma omp parallel for private(j,pivotval,k) schedule(dynamic,CHUNK)   //parallelize this portion using private variable for each thread
        for (j = i + 1; j < nsize; j++)   //j,pivotval,k are private to each thread so that there is no/less cache line bouncing
        {
            pivotval = matrix[j][i];
...
int main(int argc, char *argv[])
 {
  ...
  /*Declare variables*/
   int i,iter;       //iter is the number of threads 
 ...
 for(iter=4;iter<=104;iter=iter+4)                //run the code with different number of threads
  { 

    nsize = initMatrix(argv[1]);                 //initialize matrix
    initRHS(nsize);                               //initialize Right Hand Side
    initResult(nsize);                            //Initialize the result
...
//calculate difference from target matrix to get error percentage
  error = 0.0;
  for (i = 0; i < nsize; i++) 
  {
